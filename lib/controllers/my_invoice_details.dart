...
import '../views/my_invoice_details_response.dart';
import '../widgets/CustomAppBar.dart';
import '../widgets/main_funcation.dart';

class MyInvoiceDetailsClass extends StatefulWidget {
  final item_id;

  const MyInvoiceDetailsClass({super.key, this.item_id});
  @override
  State<MyInvoiceDetailsClass> createState() => _MyInvoiceDetailsClassState();
}

class _MyInvoiceDetailsClassState extends State<MyInvoiceDetailsClass> {
  var page_title = "My invoice details";
  var page_title2 = "";
  ...
  void initState() {
    ...
    my_invoice_details_api(widget.item_id); // Fetch initial data
    loadCartItems();
  }
  ...
  Future<void> my_invoice_details_api(var item_id) async {
    ...
    try {
      final response = await apiService.my_invoice_details_api(
        "xx",
        user_type,
        user_altercode,
        user_password,
              ...
              child: Column(
                children: [
                  Container(
                    ...
                    child: MyInvoiceDetailsResponse(json: items),
                  ),
                ],
              ),
            ),
