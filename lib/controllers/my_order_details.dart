...
class MyOrderDetailsClass extends StatefulWidget {
  ...
  const MyOrderDetailsClass({super.key, this.item_id});
  @override
...
class _MyOrderDetailsClassState extends State<MyOrderDetailsClass> {
  var page_title = "My order details";
  var page_title2 = "";
  ...
  void initState() {
    ...
    my_order_details_api(widget.item_id); // Fetch initial data
    loadCartItems();
  }
  ...
  Future<void> my_order_details_api(var item_id) async {
    ...
    try {
      final response = await apiService.my_order_details_api(
        "xx",
        user_type,
        user_altercode,
        user_password,
